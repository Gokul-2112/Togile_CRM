...
public class Baseclass {
	...
	public void setup() throws IOException, InterruptedException {
		...
		driver.manage().timeouts().implicitlyWait(Duration.ofSeconds(10));// set default wait time,when waiting time
																			// needed.
	}
	...
	public static void screenshot(String shotname, String elementshotname, WebElement element) throws IOException {
		...
		if (shotname == null) {
			...
			File desFile = new File("C:\\Users\\Admin\\git\\Togile_CRM\\Togile_CRM\\Screenshot\\" + elementshotname
					+ "--" + timestamp + ".png");
			...
			File desFile = new File("C:\\Users\\Admin\\git\\Togile_CRM\\Togile_CRM\\Screenshot\\" + shotname + "--"
					+ timestamp + ".png");
			FileUtils.copyFile(SourceFile, desFile);
		}
	...
	// Helper method to extract plain text from MimeMultipart (for multipart
	// messages)
	...
	public static void validation(String placeholderText, String expectedErrorMsg) {
		...
		WebElement error = wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//input[@placeholder='"
				+ placeholderText + "']/following-sibling::div[contains(@class, 'invalid-feedback')]")));
	...
	public static void sorting(List<WebElement> columnCells, String headerName) throws InterruptedException {
		...
		if (isNumeric) {
		    ...
		    List<Long> actual = columnData.stream()
		            .map(s -> s.replaceAll("\\D", "")) // remove +, spaces, etc.
		            .map(Long::parseLong)
		    ...
		    if (actual.equals(expected) && !actual.isEmpty()) {
		        System.out.println("PASS: " + headerName + " is sorted correctly.");
		    } else if (actual.isEmpty()) {
		        System.out.println("Fail: " + headerName + " is Empty.");
		    } else {
		    ...
		    // Check if it's a Date-Time column
		        ...
		        for (String s : columnData) {
		            try {
		                actualDates.add(formatter.parse(s));
		            } catch (ParseException e) {
		                e.printStackTrace();
		            }
		        }
		        ...
		        expectedDates.sort(Collections.reverseOrder()); // Sort newest first

		        if (actualDates.equals(expectedDates) && !actualDates.isEmpty()) {
		            System.out.println("PASS: " + headerName + " is sorted correctly.");
		        } else if (actualDates.isEmpty()) {
		            System.out.println("Fail: " + headerName + " is Empty.");
		        } else {
		            System.out.println("FAIL: " + headerName + " is not sorted correctly.");
		        }
			...
			if (headerName.equalsIgnoreCase("First Name")|| headerName.equalsIgnoreCase("Name")){
			...
			// Compare
			if (columnData.equals(expected) && !columnData.isEmpty()) {
				System.out.println("PASS: " + headerName + " is sorted correctly.");
